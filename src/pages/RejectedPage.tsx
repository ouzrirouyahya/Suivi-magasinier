    ...
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-6 text-slate-100">
      ...
      >
        ...
        <div className="space-y-2">
          <h2 className="text-2xl font-bold tracking-tight text-white">Demande d'accès rejetée</h2>
          <p className="text-slate-400 text-sm">
            Désolé, <span className="font-semibold text-slate-200">{currentUser?.name || currentUser?.email}</span>.
          </p>
          <p className="text-slate-400 text-sm">
            Votre demande d'inscription pour le rôle de 
            <span className="font-semibold text-slate-300"> {currentUser?.requestedRole || 'MAGASINIER'}</span> a été refusée par l'administrateur système.
          </p>
        </div>
