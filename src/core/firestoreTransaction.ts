 ...
 * File: /src/core/firestoreTransaction.ts
 *
...
import { registerSuccessAck } from './operationIntent';
...
export function classifyFirestoreError(err: any): 'FATAL' | 'RETRYABLE' {
  ...
  switch (code) {
    ...
    default:
      ...
      // General fallbacks: err.name = "FirebaseError" is generic, default to RETRYABLE for robustness
      return 'RETRYABLE';
  }
}
...
export async function executeIdempotentTransaction<T>(
  ...
  try {
    ...
    if (finalResult.status === 'SUCCESS_ACK' || finalResult.status === 'IDEMPOTENT_NOOP') {
      registerSuccessAck(intentId);
    }
